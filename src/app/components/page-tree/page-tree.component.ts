...
export class PageTreeComponent implements OnInit
  ...
  visibleNodes(): FileNode[] {
    ...
    function addExpandedChildren(node: FileNode, expanded: string[]): any {
      result.push(node);
  ...
  drop(event: CdkDragDrop<string[]>): void {
    ...
    function findNodeByID(arr: Array<FileNode>, id: string): any {
      let rv;
      arr.forEach((n) => {
        if (n.id === id) {
          rv = n;
        } else if (n.children) {
          rv = rv || findNodeByID(n.children, id);
        }
      });
      return rv;
      }
    ...
    // insert node
    if (this.insert) {
      if (nodeAtDest.children) {
        findNodeByID(newSiblings, nodeAtDest.id).children.push(nodeToInsert);
      } else{
        findNodeByID(newSiblings, nodeAtDest.id).children = [nodeToInsert];
      }
    ...
    const temp = findNodeByID(changedData, nodeAtDest.id);
   ...
   */
  private expandNodesById(flatNodes: FileFlatNode[], ids: string[]): void {
